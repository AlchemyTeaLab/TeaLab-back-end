...
module.exports = Router()
  ...
  .post('/', authenticate, async (req, res, next) => {
    ...
    try {
      const newRecipe = await Recipe.insert(req.body.recipe);
      await Promise.all(
        req.body.recipeItems.map((i) => Recipe.addIngredientsById(i, newRecipe.id))
        // console.log('i log', i, 'newRecipe', newRecipe.id))
      );
      res.send(newRecipe);
    } catch (error) {
      next(error);
